...
void etf_get_initial(etf_model_t model,int* state){
	...
	if (!model->initial_state){
	  fprintf(stderr, "model has no initial state");
	}
...
void etf_ode_add(etf_model_t model){
  if (model->trans_count) fprintf(stderr,"model already has transitions");
	...
	if (state_length != model->map_count){
		fprintf(stderr,"inconsistent map count");
	}
	int vcount[state_length];
	for(int i=0;i<state_length;i++){
		vcount[i]=SIgetCount(model->type_values[i]);
		fprintf(stderr,"var %d is %s with %d values",i,lts_type_get_state_name(model->ltstype,i),vcount[i]);
	}
	int is_new;
	int signtype=lts_type_add_type(model->ltstype,"sign",&is_new);
	if(is_new) fprintf(stderr,"model does not define sign type");
...
//		Warning(debug,"map is consistent");
		...
		if(varidx>=state_length){
			fprintf(stderr,"variable %s is not a state variable",var);
		} else {
		...
		while(ETFmapNext(map,state,&value)){
			...
			do {
				...
				if (value!=neg && value!=pos) {
					fprintf(stderr,"unexpected sign: %d",value);
				}
				char label[1024];
