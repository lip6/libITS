...
\usepackage[figure,vlined,linesnumbered,inoutnumbered,commentsnumbered]{algorithm2e}
\usepackage{url}
\usepackage{hyperref}
...
\section{Algorithmes symboliques}

La figure~\ref{fig:owcty} présente l'algorithme OWCTY. Il a été modifié par rapport à celui présenté dans~\cite{FFKV01}. Les parcours ont été inversés pour être en avant. Ceci a pour conséquence que les ensembles d'états manipulés sont toujours clos vers le bas (i.e. si un état appartient à l'ensemble alors ses successeurs y appartiennent aussi). Les fonctions \texttt{Post} et \texttt{Sat} ne peuvent donc pas sortir de l'ensemble $B$ et cela nous permet de supprimer quelques intersections. Enfin, l'ordre des deux boucles internes a été inversé.
...
\caption{Algorithme EL en parcours avant.\label{fig:el}}
\end{algorithm}
...
\item Comment prendre en compte les résultats de~\cite{GH06} dans Spot ?
