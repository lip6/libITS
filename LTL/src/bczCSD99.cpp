...
spot::state* bcz_succ_iterator::dst() const {
  assert(!done());
...
bdd bcz_succ_iterator::cond() const {
  assert(!done());
...
spot::acc_cond::mark_t bcz_succ_iterator::acc() const {
  assert(!done());
  return {};
} //
