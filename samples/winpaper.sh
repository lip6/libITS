...
grep -vE SLAP-FSA $f | grep -v 'X[^W]' | ./winner.pl -l 7 - |
  sed 's/hline/hline\\multirow{6}{*}{\\begin{sideways}without $\\X$\\end{sideways}}/;s/FS\([OE]\)/\1/g'
grep -vE SLAP-FSA $f | (echo; grep 'X[^W]') | ./winner.pl -l 7 - |
  sed '1d;s/hline/hline\\multirow{6}{*}{\\begin{sideways}with $\\X$\\end{sideways}}/;s/FS\([OE]\)/\1/g'
...
grep -vE SLAP-FSA $f | grep -v 'X[^W]' | ./winner.pl -a 7 - |
  sed 's/hline/hline\\multirow{6}{*}{\\begin{sideways}w\/o $\\X$\\end{sideways}}/;s/FS\([OE]\)/\1/g'
